...
function simulateContractInteraction() {
    ...
    const interactions = [
      { type: 'Transfer', from: generateRandomSolanaAddress(), to: generateRandomSolanaAddress(), amount: `${generateRandomAmount()} ${generateRandomTokenSymbol()}` },
      { type: 'Mint', to: generateRandomSolanaAddress(), amount: `${generateRandomAmount()} ${generateRandomTokenSymbol()}` },
      { type: 'Transfer', from: generateRandomSolanaAddress(), to: generateRandomSolanaAddress(), amount: `${generateRandomAmount()} ${generateRandomTokenSymbol()}` }
    ...
    listItem.innerHTML = `
      <div class="bg-gray-800 p-4 rounded-lg mb-2 flex items-center justify-between">
        <div>
          ...
          ${newInteraction.from ? `<p class="text-sm">From: <span class="font-mono">${newInteraction.from}</span></p>` : ''}
  ...
  function updateBribeUpdates() {
    ...
    bribeItem.className = "flex justify-between items-center bg-gray-800 p-4 rounded-lg mb-2";
    bribeItem.innerHTML = `
